...
void app_loop(void)
{
    ...
    if(app_lora_status.IsJoined)  //if LoRaWAN is joined
    {
        if(autosend_flag) 
        {
            ...
            lpp_data[lpp_cnt].startcnt = sensor_data_cnt;
            a[sensor_data_cnt++]=0x09;
            a[sensor_data_cnt++]=0x45;
            a[sensor_data_cnt++]=getPumpState();
            lpp_data[lpp_cnt].size = sensor_data_cnt - lpp_data[lpp_cnt].startcnt;
            lpp_cnt++;
...
void LoRaReceive_callback(RUI_RECEIVE_T* Receive_datapackage)
{
    int match = 0;
    char hex_str[3] = {0}; 
    RUI_LOG_PRINTF("at+recv+lora=%d,%d,%d,%d", Receive_datapackage->Port, Receive_datapackage->Rssi, Receive_datapackage->Snr, Receive_datapackage->BufferSize);   
    ...
    if ((Receive_datapackage->Buffer != NULL) && Receive_datapackage->BufferSize) {
        ...
        for (int i = 0; i < Receive_datapackage->BufferSize; i++) {
            ...
            RUI_LOG_PRINTF("%s\r\n", hex_str); 
            if(i==0 && strcmp(hex_str,"09") == 0) match++;
            if(i==1 && strcmp(hex_str,"45") == 0) match++;
            if(i==2 && strcmp(hex_str,"ff") == 0) match++;
        }
        
        if( match == 3){
            RUI_LOG_PRINTF("PumpStart\r\n"); 
            pumpStart();
        } else {
            RUI_LOG_PRINTF("PumpStop\r\n"); 
            pumpStop();
        } 
    }
    RUI_LOG_PRINTF("Buffer Size %d\r\n", Receive_datapackage->BufferSize); 
...
{
    char hex_str[3]={0};
    RUI_LOG_PRINTF("at+recv+p2p=%d,%d,%d:", Receive_P2Pdatapackage -> Rssi, Receive_P2Pdatapackage -> Snr, Receive_P2Pdatapackage -> BufferSize); 
    for(int i=0;i < Receive_P2Pdatapackage -> BufferSize; i++)
    {
        ...
        if(Receive_P2Pdatapackage->BufferSize >= 3){
            if(Receive_P2Pdatapackage->Buffer[0] == 0x09 && Receive_P2Pdatapackage->Buffer[1] == 0x45){
                RUI_LOG_PRINTF("Address correct\r\n"); 
                if(Receive_P2Pdatapackage->Buffer[2] == 0xFF){
                    pumpStart();
                } else {
                    pumpStop();
                }
